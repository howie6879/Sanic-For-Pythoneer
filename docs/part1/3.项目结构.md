...
具体结构代码见[sample01](https://github.com/howie6879/Sanic-For-Pythoner/tree/master/examples/demo03/sample01)，运行起来，然后输入 `http://0.0.0.0:8000/html` 就可以看到页面被展示出来了^_^

如果需要编写前端页面比较多，那么你就需要添加`static` 以及 `templates` 文件夹用来管理各个界面模块，具体下面会介绍。
...
每个项目的内部分布以及命名可能不一样（甚至目录比应该或多或少），但大体意思可能差不多，下面介绍下 本次项目`src` 下的一些文件目录结构：
